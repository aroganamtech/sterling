        ...
        {groups.map((g, i) => (
          <button
            ...
            className={`relative px-6 py-4 text-[11.5px] font-semibold uppercase tracking-[0.13em] transition-colors ${
              i === active ? 'text-ember-400' : 'text-ink-400 hover:text-ink-200'
            }`}
          >
            {g.group}
            {i === active ? (
              <motion.span
                layoutId="spec-tab"
                className="absolute inset-x-0 bottom-0 h-[2px] bg-ember-500"
        ...
        >
          {group.rows.map(([k, v]) => (
            <div key={k} className="grid gap-1 px-6 py-4 sm:grid-cols-[minmax(0,240px)_1fr] sm:gap-8">
              <dt className="text-[13px] uppercase tracking-[0.08em] text-ink-400">{k}</dt>
